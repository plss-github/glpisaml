...
// WebPaths
define(
    "PLUGIN_GLPISAML_ATOM_URL",
    "https://codeberg.org/QuinQuies/glpisaml/releases.rss",
); // Location of the repository versions
 ...
 */
function plugin_init_glpisaml(): void
{
    //NOSONAR - phpcs:ignore PSR1.Function.CamelCapsMethodName
    ...
    if (
        $plugin->isInstalled(PLUGIN_NAME) ||
        ...
        if (Session::haveRight("config", UPDATE)) {
            $PLUGIN_HOOKS["config_page"][
                PLUGIN_NAME
            ] = PLUGIN_GLPISAML_CONF_PATH; //NOSONAR
        }
        $PLUGIN_HOOKS["menu_toadd"][PLUGIN_NAME]["config"] = [Config::class];
        $PLUGIN_HOOKS[Hooks::ADD_CSS][
            PLUGIN_NAME
        ][] = PLUGIN_GLPISAML_CONFCSS_PATH;

        // Register and hook the saml rules
        Plugin::registerClass(RuleSamlCollection::class, [
        ...
        $PLUGIN_HOOKS[Hooks::POST_INIT][PLUGIN_NAME] =
            "plugin_glpisaml_evalAuth"; //NOSONAR

        // Hook the login buttons
        $PLUGIN_HOOKS[Hooks::DISPLAY_LOGIN][PLUGIN_NAME] =
            "plugin_glpisaml_displaylogin";
    }
}
 ...
 * @return array
 */
function plugin_version_glpisaml(): array
{
    //NOSONAR - phpcs:ignore PSR1.Function.CamelCapsMethodName
    return [
        "name" => "GLPI SAML",
        ...
        "author" => "Ampris",
        "license" => "GPLv2+",
        "homepage" => "https://github.com/plss-github/saml-glpi-plugin.git",
        "requirements" => [
            "glpi" => [
 ...
 * @return boolean
 */
function plugin_glpisaml_check_prerequisites(): bool
{
    //NOSONAR - phpcs:ignore PSR1.Function.CamelCapsMethodName
 ...
 * @return boolean
 */
function plugin_glpisaml_check_config($verbose = false): bool
{
    //NOSONAR - phpcs:ignore PSR1.Function.CamelCapsMethodName
    if ($verbose) {
